...
# **Introduction**
In this report, we will explore the question: is there an association between political affiliation (either conservative or liberal) and whether a person considers themselves an environmentalist or not? To achieve this, a contingency table was created with the counts and proportions of responses to the question above and then chi-square test for independence was performed to assess the association between political party and environmentalism. The data used is from a survey conducted by Chelsea Batavia and Andrews Forest to explore the "influences on charitable giving for conservation." The survey included 1,331 respondents and was conducted in August 2017. 
...
Following the analysis, a significant association was found between the political party a person is affiliated with and whether or not they consider themselves an environmentalist: ($\chi$^2^(`r environ_tidy$parameter`) = `r round(environ_tidy$statistic, 2)`, *p* `r ifelse(environ_tidy$p.value < 0.001, paste("<0.001"), paste("=", environ_tidy$p.value))`)
